...
public class Genetic_Labour {
	...
	private void setFitnesses() {
		for (Map map : currentGeneration) {
			if(currentLowestFitness == null) {
				...
				secondBestFitness = currentLowestFitness;
				currentLowestFitness = map;
			} else if (map.getFitness() > currentLowestFitness.getFitness() && map.getFitness() < secondBestFitness.getFitness()) {
				secondBestFitness = map;
			}
		}
	}
	...
	public Map generateMaps() {
		...
		while (currentLowestFitness.getFitness() > 10) {
//			System.out.println(currentGeneration.size());
			// größe größer 2
			int iterator = 0;
			int crossDivider = new Random().nextInt(currentLowestFitness.getCellList()
					.size());
...
//			System.out.printf("Fitness: %f\n", childMap.getFitness());
		...
		System.out.println("Number of Generations: " + xyz + " Time needed: " + (System.currentTimeMillis() - time));
